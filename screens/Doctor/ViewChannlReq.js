...
function ViewChannlReq({navigation}) {
    ...
    async function loadRequests(){ 
        ...
        const query = channelRef.where('did', '==', await AsyncStorage.getItem('uid'));
    ...
    function handleSearch(){
        ...
        if(searchValue != ""){
            for(let r of requestList){
                if(r.data.doc_name.toLowerCase().startsWith(searchValue.toLowerCase())){
                    tempList.push(r);
                }
            }
            setFilteredRequestsList(tempList);
                ...
                <ScrollView >
                        {filteredRequestsList.map(
                            ...
                            <Pressable style={styles.doctorCard}  key={req.id} android_ripple={{color:'#e3e3e3'}}  onPress={() => {setselectedRequest(req); setIsVisible(true)}}  >
                                <View >
                                    <Text style={{color:'white', fontWeight:'bold', fontSize:20}}>{req.data.doc_name}</Text>
            ...
            <Modal
                ...
                <View style={styles.modalContainer}>
                    ...
                    <View style={{justifyContent:'center', alignItems:'center', borderBottomWidth:1, borderColor:'white',marginBottom:20}}>
                        <Text style={{color:'white', fontWeight:'bold', fontSize:25, marginBottom:20}}>Channeling Details</Text>
                    </View>

                    <View style={{paddingHorizontal:30, marginBottom:20}}>
                        <Text style={{color:'white', fontSize:18, fontWeight:'bold'}}>Doctor's Name : </Text>
                        <Text style={{color:'white', }}>{selectedRequest.data.doc_name}</Text>
                    </View>
                    <View style={{paddingHorizontal:30, marginBottom:20}}>
                    ...
                    <View style={{paddingHorizontal:30, marginBottom:20}}>
                        ...
                        <Text style={{color:'white', }}>{selectedRequest.data.time}</Text>
                    </View>
